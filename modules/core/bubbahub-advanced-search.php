    ...
    <div class="bh-directory" data-ajax-url="<?php echo esc_url(admin_url('admin-ajax.php'));?>" data-nonce="<?php echo esc_attr($nonce);?>">
      <form class="bh-search-form" method="get">
        ...
        <div class="bh-advanced-search" hidden>
          <div class="bh-filter-option"><span class="bh-filter-title">Choose a region</span><label class="screen-reader-text" for="bh-region">Choose a region</label><select id="bh-region" name="bh_region"><option value="">All regions</option><?php if(!is_wp_error($regions))foreach($regions as $t):?><option value="<?php echo esc_attr($t->slug);?>" <?php selected($f['region'],$t->slug);?>><?php echo esc_html($t->name);?></option><?php endforeach;?></select></div>

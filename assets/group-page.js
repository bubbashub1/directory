...
function init(){var $root=$('.bhg-single');if(!$root.length)return;
 ...
 $root.on('click','[data-badge-action]',function(){var $b=$(this),a=$b.data('badge-action'),id=String($root.data('post-id'));if(a==='favourite'||a==='visited'){if(typeof BubbaHubFavourites==='undefined'){return;}var action=a==='favourite'?'bubbahub_toggle_favourite':'bubbahub_toggle_visited';var data=new FormData();data.append('action',action);data.append('nonce',BubbaHubFavourites.nonce);data.append('group_id',id);$b.prop('disabled',true);fetch(BubbaHubFavourites.ajaxUrl,{method:'POST',body:data,credentials:'same-origin'}).then(function(r){return r.json();}).then(function(r){if(!r.success){if(r.data&&r.data.message)alert(r.data.message);return;}var list=read(a),i=list.indexOf(id);if(r.data.saved&&i===-1)list.push(id);if(!r.data.saved&&i!==-1)list.splice(i,1);write(a,list);sync();}).catch(function(){}).finally(function(){$b.prop('disabled',false);});return;}var list=read(a),i=list.indexOf(id);if(i===-1)list.push(id);else list.splice(i,1);write(a,list);sync();});

...
function bubbahub_leader_handle_booking_save() {
	if ( empty($_POST['bh_leader_action']) || $_POST['bh_leader_action'] !== 'save_booking' || !is_user_logged_in() ) return;
	...
	$booking_id=absint($_POST['booking_id']??0); $group_id=absint($_POST['group_id']??0);
	...
	$data=array('post_type'=>'bh_booking','post_status'=>'publish','post_title'=>sanitize_text_field(wp_unslash(($_POST['first_name']??'').' '.($_POST['last_name']??''))));
	if($booking_id){$data['ID']=$booking_id;$saved=wp_update_post(wp_slash($data),true);}else{$data['post_author']=get_current_user_id();$saved=wp_insert_post(wp_slash($data),true);}
	if(is_wp_error($saved)) return;
	$map=array('_bh_group_id'=>$group_id,'_bh_first_name'=>sanitize_text_field(wp_unslash($_POST['first_name']??'')),'_bh_last_name'=>sanitize_text_field(wp_unslash($_POST['last_name']??'')),'_bh_email'=>sanitize_email($_POST['email']??''),'_bh_booking_date'=>sanitize_text_field($_POST['booking_date']??''),'_bh_total_places'=>absint($_POST['total_places']??1),'_bh_total_price'=>wc_format_decimal($_POST['total_price']??0),'_bh_status'=>sanitize_key($_POST['booking_status']??'pending'),'_bh_payment_status'=>'pending');
	foreach($map as $k=>$v) update_post_meta($saved,$k,$v);
	wp_safe_redirect(add_query_arg('booking_saved','1',home_url('/leader/#bookings'))); exit;
}

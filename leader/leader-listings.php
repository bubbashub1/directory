...
function bubbahub_leader_handle_listing_save() {
	if ( empty( $_POST['bh_leader_action'] ) || $_POST['bh_leader_action'] !== 'save_listing' || ! is_user_logged_in() ) return;
	...
	$data = array( 'post_type' => 'group', 'post_title' => $title, 'post_content' => wp_kses_post( wp_unslash( $_POST['post_content'] ?? '' ) ), 'post_status' => $status );
	...
	if ( ! is_wp_error( $saved ) ) wp_safe_redirect( add_query_arg( 'listing_saved', '1', home_url( '/leader/#listings' ) ) );
	exit;
}

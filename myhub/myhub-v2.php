...
function bubbahub_myhub_v3_school_status_label( $status ) {
    $labels = array(
        ...
        'active'            => 'Active',
        'submitted'         => 'Submitted',
        'accepted_attending'=> 'Accepted & Attending',
        'complete'          => 'Complete',
    );
    return isset( $labels[ $status ] ) ? $labels[ $status ] : ( $status ? ucwords( str_replace( '_', ' ', $status ) ) : 'Active' );
}
...
function bubbahub_myhub_v3_auto_school_deadline( $dob ) {
    ...
    // Reception entry year: children born 1 Sep–31 Aug enter in the school year
    // beginning after their fifth birthday. This is used only as a fallback when
    // no explicit ACF deadline has been saved.
    $entry_year = (int) $birth->format( 'Y' ) + 5;
    if ( (int) $birth->format( 'm' ) > 8 || ( (int) $birth->format( 'm' ) === 8 && (int) $birth->format( 'd' ) > 31 ) ) {
        $entry_year++;
    }
...
function bubbahub_myhub_v3_school_tracker( $child_id, $status, $dob ) {
    ...
    // Keep the tracker useful for existing child profiles even when the new
    // deadline field has not yet been populated in ACF.
    if ( ! $deadline && $dob ) $deadline = bubbahub_myhub_v3_auto_school_deadline( $dob );
    ...
    <div class="bh-myhub-tracker bh-myhub-school-tracker <?php echo esc_attr( $tracker_class ); ?>">
        ...
        <?php if ( $has_school ) : ?>
            ...
            <?php endif; ?>
        <?php elseif ( $deadline ) : ?>
            <strong>Application deadline: <?php echo esc_html( wp_date( 'j M Y', strtotime( $deadline ) ) ); ?></strong>
        <?php else : ?>
            <strong>School application details not added yet</strong>
        <?php endif; ?>

        <?php if ( $countdown && ! $has_school && 'accepted_attending' !== $school_status && 'complete' !== $school_status ) : ?>
        ...
        <section class="bh-myhub-section">
            ...
            <div class="bh-myhub-family-grid">
                <?php if ( $children ) : foreach ( $children as $child ) :
                    ...
                    $is_expecting = ( 'expecting' === $status && $due );
                    ?>
                    <article class="bh-myhub-child-card">
                        ...
                        <?php if ( $is_expecting ) : ?>
                            <div class="bh-myhub-tracker pregnancy">
                                ...
                                <span>Due <?php echo esc_html( wp_date( 'j M Y', strtotime( $due ) ) ); ?></span>
                            </div>
                        <?php else : ?>

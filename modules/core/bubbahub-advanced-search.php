...
function bubbahub_advanced_search_shortcode($output,$tag,$attr,$m){
    ...
    $regions=get_terms(array('taxonomy'=>'region','hide_empty'=>false));$cat_tax=bubbahub_advanced_search_category_tax();$categories=$cat_tax?get_terms(array('taxonomy'=>$cat_tax,'hide_empty'=>false)):array();$acf=bubbahub_advanced_search_acf_fields();
    ...
    <div class="bh-directory" data-ajax-url="<?php echo esc_url(admin_url('admin-ajax.php'));?>" data-nonce="<?php echo esc_attr($nonce);?>">
      <form class="bh-search-form" method="get">
        ...
        <div class="bh-advanced-search" hidden>
          <div><label for="bh-region">Region</label><select id="bh-region" name="bh_region"><option value="">All regions</option><?php if(!is_wp_error($regions))foreach($regions as $t):?><option value="<?php echo esc_attr($t->slug);?>" <?php selected($f['region'],$t->slug);?>><?php echo esc_html($t->name);?></option><?php endforeach;?></select></div>
          <div><label for="bh-category">Category</label><select id="bh-category" name="bh_category"><option value="">All categories</option><?php foreach((array)$categories as $t):?><option value="<?php echo esc_attr($t->slug);?>" <?php selected($f['category'],$t->slug);?>><?php echo esc_html($t->name);?></option><?php endforeach;?></select></div>
          <div><label for="bh-day">Day</label><select id="bh-day" name="bh_day"><option value="">Any day</option><?php foreach($days as $v=>$label):?><option value="<?php echo esc_attr($v);?>" <?php selected($f['day'],$v);?>><?php echo esc_html($label);?></option><?php endforeach;?></select></div>
          <div><label for="bh-term-time">Term Time</label><select id="bh-term-time" name="bh_term_time"><option value="">Any time of year</option><option value="yes" <?php selected($f['term_time'],'yes');?>>Term time only</option><option value="no" <?php selected($f['term_time'],'no');?>>Not term time only</option></select></div>
          <div><label for="bh-age">Age</label><select id="bh-age" name="bh_age"><option value="">All ages</option><?php foreach((array)bubbahub_directory_age_values() as $v):?><option value="<?php echo esc_attr($v);?>" <?php selected($f['age'],$v);?>><?php echo esc_html($v);?></option><?php endforeach;?></select></div>
          <div><label for="bh-price">Price</label><select id="bh-price" name="bh_price"><option value="">Any price</option><option value="free" <?php selected($f['price'],'free');?>>Free</option><option value="paid" <?php selected($f['price'],'paid');?>>Paid</option></select></div>
          <?php foreach($acf as $name=>$field):$choices=bubbahub_advanced_search_acf_fields()[$name]['choices']??array();if('true_false'===$field['type'])$choices=array('1'=>'Yes','0'=>'No');?><div class="bh-acf-filter"><label for="bh-acf-<?php echo esc_attr($name);?>"><?php echo esc_html($field['label']?:ucwords(str_replace('_',' ',$name)));?></label><select id="bh-acf-<?php echo esc_attr($name);?>" name="bh_acf[<?php echo esc_attr($name);?>]"><option value="">Any</option><?php foreach((array)$choices as $v=>$label):?><option value="<?php echo esc_attr($v);?>" <?php selected($f['acf'][$name]??'',$v);?>><?php echo esc_html($label);?></option><?php endforeach;?></select></div><?php endforeach;?>
          <div><label for="bh-radius">Location radius</label><select id="bh-radius" name="bh_radius"><option value="5">5 miles</option><option value="10">10 miles</option><option value="25" selected>25 miles</option><option value="50">50 miles</option></select></div>
        </div>

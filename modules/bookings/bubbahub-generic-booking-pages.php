...
final class BubbaHub_Generic_Booking_Pages {
    ...
    public function register_cpt() {
        register_post_type( self::CPT, array(
            'labels' => array( 'name'=>'Booking Pages','singular_name'=>'Booking Page','add_new_item'=>'Add Booking Page','edit_item'=>'Edit Booking Page' ),
            'public' => true, 'show_ui' => false, 'show_in_menu' => false, 'supports' => array( 'title','editor','author' ),
            'rewrite' => array( 'slug' => 'booking-page', 'with_front' => false ), 'has_archive' => false,
        ) );
    }

    private function owned( $id ) {
        return $id && self::CPT === get_post_type( $id ) && (int) get_post_field( 'post_author', $id ) === get_current_user_id();
    }
    ...
    public function handle_save() {
        ...
        $content=wp_kses_post(wp_unslash($_POST['page_description']??''));
        $post=array('post_type'=>self::CPT,'post_title'=>$title,'post_content'=>$content,'post_status'=>'publish');

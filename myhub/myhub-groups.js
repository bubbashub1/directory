...
function initWidget($widget){var type=$widget.data('group-type'),viewMore=$widget.data('view-more'),ids=type==='suggested'?[]:list(type);request($widget,type,ids,true);var $link=$widget.closest('.bh-myhub-group-column').find('[data-group-view-more]');if($link.length&&viewMore)$link.attr('href',BubbaHubMyHubGroups.pageUrl+'?group_view='+encodeURIComponent(type));}
function initPage($page){var type=$page.find('[data-myhub-group-results]').data('group-view'),ids=type==='suggested'?[]:list(type);request($page.find('[data-myhub-group-results]'),type,ids,false);}
$(function(){if(typeof BubbaHubMyHubGroups==='undefined')return;$('[data-myhub-group-widget]').each(function(){initWidget($(this));});$('[data-myhub-groups-page]').each(function(){initPage($(this));});});
})(jQuery);

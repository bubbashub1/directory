...
define( 'BUBBAHUB_DIRECTORY_VERSION', '1.2.0' );
...
function bubbahub_directory_normalise_map( $map ) {
    ...
    if ( is_string( $map ) && preg_match( '/(-?\d+(?:\.\d+)?)[, ]+(-?\d+(?:\.\d+)?)/', $map, $m ) ) return array( 'lat' => (float) $m[1], 'lng' => (float) $m[2] );
    return null;
}
...
function bubbahub_directory_shortcode( $atts ) {
    ...
    $query = bubbahub_directory_query( $filters ); $regions = get_terms( array( 'taxonomy' => 'region', 'hide_empty' => true ) ); $age_values = bubbahub_directory_age_values();
    wp_enqueue_style( 'bubbahub-directory' ); wp_enqueue_style( 'leaflet' ); wp_enqueue_script( 'leaflet' ); wp_enqueue_script( 'bubbahub-directory' );
    wp_localize_script( 'bubbahub-directory', 'BubbaHubDirectory', array( 'ajaxUrl' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce( 'bubbahub_directory' ), 'postsPerPage' => (int) $atts['posts_per_page'] ) );
    ob_start(); ?>
    <div class="bh-directory" data-default-columns="4">
        <form class="bh-searchbar" method="get"><div class="bh-search-main"><label class="bh-search-field"><span class="bh-sr-only">Search</span><input type="search" name="bh_search" value="<?php echo esc_attr( $filters['search'] ); ?>" placeholder="Search groups, classes & activities..."></label><button class="bh-search-button" type="submit">Search</button><button class="bh-advanced-toggle" type="button" aria-expanded="false">Advanced search <span aria-hidden="true">⌄</span></button></div>
        <div class="bh-advanced" hidden><div><label for="bh-region">Location</label><select id="bh-region" name="bh_region"><option value="">All locations</option><?php if ( ! is_wp_error( $regions ) ) foreach ( $regions as $term ) : ?><option value="<?php echo esc_attr( $term->slug ); ?>" <?php selected( $filters['region'], $term->slug ); ?>><?php echo esc_html( $term->name ); ?></option><?php endforeach; ?></select></div><div><label for="bh-age">Age range</label><select id="bh-age" name="bh_age"><option value="">All ages</option><?php foreach ( $age_values as $value ) : ?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $filters['age'], $value ); ?>><?php echo esc_html( $value ); ?></option><?php endforeach; ?></select></div><div><label for="bh-price">Price</label><select id="bh-price" name="bh_price"><option value="">Any price</option><option value="free" <?php selected( $filters['price'], 'free' ); ?>>Free</option><option value="paid" <?php selected( $filters['price'], 'paid' ); ?>>Paid</option></select></div><button type="submit" class="bh-apply">Apply filters</button></div></form>
        <div class="bh-toolbar"><div class="bh-results-count" aria-live="polite"><strong><?php echo esc_html( number_format_i18n( $query->found_posts ) ); ?></strong> groups</div><div class="bh-view-controls" role="group" aria-label="Directory view"><span class="bh-view-label">View</span><?php foreach ( array( 2,3,4,5,6 ) as $columns ) : ?><button type="button" class="bh-cols <?php echo $columns === 4 ? 'is-active' : ''; ?>" data-columns="<?php echo $columns; ?>" aria-label="<?php echo $columns; ?> columns"><?php echo $columns; ?></button><?php endforeach; ?><button type="button" class="bh-map-toggle" data-view="map">Map</button></div></div>
        <div class="bh-loading" hidden><span class="bh-spinner" aria-hidden="true"></span> Updating groups…</div><div class="bh-content"><div class="bh-grid" data-columns="4"><?php echo bubbahub_directory_render_cards( $query ); ?></div><div class="bh-map" id="bh-directory-map" hidden aria-label="Map showing groups"></div></div><nav class="bh-pagination" aria-label="Directory pagination"><?php echo bubbahub_directory_render_pagination( $query ); ?></nav>
    </div>

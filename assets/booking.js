...
ready(function(){
 ...
 function formatDate(v){var d=new Date(v+'T12:00:00');return isNaN(d.getTime())?v:d.toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'});}
 function post(data){var body=new URLSearchParams();Object.keys(data).forEach(function(k){body.append(k,data[k]);});return fetch(BubbaHubBookingUI.ajaxUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'},body:body.toString(),credentials:'same-origin'}).then(function(r){return r.json();});}
 function openModal(){lastFocused=document.activeElement;modal.hidden=false;modal.setAttribute('aria-hidden','false');document.body.classList.add('bh-booking-modal-open');var c=modal.querySelector('.bh-booking-modal-close');if(c)c.focus();if(dateSelect&&!dateSelect.value&&dateSelect.options.length>1){dateSelect.value=dateSelect.options[1].value;load(dateSelect.value);}}
 function closeModal(){modal.hidden=true;modal.setAttribute('aria-hidden','true');document.body.classList.remove('bh-booking-modal-open');if(reservePanel)reservePanel.hidden=true;if(lastFocused&&lastFocused.focus)lastFocused.focus();}
 document.querySelectorAll('[data-booking-open]').forEach(function(t){t.addEventListener('click',function(e){e.preventDefault();openModal();});});
 ...
 function selectSession(id,btn){
   ...
   var action=selected.booking_action||((selected.booking_method==='external')?'external':(selected.booking_method==='none'?'none':(selected.reserve_enabled?'reserve_spot':'book_now')));
   if(action==='book_now'){
     var book=document.createElement('button');book.type='button';book.className='bh-booking-primary';book.textContent='Book Now';book.addEventListener('click',function(){window.dispatchEvent(new CustomEvent('bubbahub:booking-selected',{detail:selected}));showMessage('Booking form selected.');});actionButtons.appendChild(book);
   }else if(action==='reserve_spot'){
     var reserve=document.createElement('button');reserve.type='button';reserve.className='bh-booking-primary';reserve.textContent='Reserve Spot';reserve.addEventListener('click',function(){reservePanel.hidden=false;reservePanel.scrollIntoView({behavior:'smooth',block:'nearest'});});actionButtons.appendChild(reserve);
   }else if(action==='external'){
     if(selected.external_url){var ext=document.createElement('a');ext.className='bh-booking-primary';ext.href=selected.external_url;ext.target='_blank';ext.rel='noopener noreferrer';ext.textContent='External Booking →';actionButtons.appendChild(ext);}else{var missing=document.createElement('span');missing.className='bh-booking-message is-error';missing.textContent='External booking has been selected, but no booking link has been added.';actionButtons.appendChild(missing);}
   }
   actions.hidden=action==='none';
 }
 function showMessage(text,error){var box=root.querySelector('[data-reserve-message]');if(box){box.textContent=text;box.className='bh-booking-message'+(error?' is-error':'');}}
 ...
 var reserveSubmit=root.querySelector('[data-reserve-submit]');if(reserveSubmit)reserveSubmit.addEventListener('click',function(){if(!selected)return;var name=root.querySelector('[data-reserve-name]').value.trim(),email=root.querySelector('[data-reserve-email]').value.trim(),places=parseInt(root.querySelector('[data-reserve-places]').value,10)||1;showMessage('');reserveSubmit.disabled=true;reserveSubmit.textContent='Reserving…';post({action:'bubbahub_booking_reserve',nonce:BubbaHubBookingUI.nonce,session_id:selected.id,customer_name:name,customer_email:email,places:places}).then(function(res){if(!res.success)throw new Error((res.data&&res.data.message)||'Reservation failed.');showMessage(res.data.message+' Reference #'+res.data.booking_id);root.querySelector('[data-reserve-name]').value='';root.querySelector('[data-reserve-email]').value='';load(dateSelect.value);}).catch(function(err){showMessage(err.message||'Reservation failed.',true);}).finally(function(){reserveSubmit.disabled=false;reserveSubmit.textContent='Reserve Spot';});});
});
})();

        ...
        <div class="bhg-shell">
            ...
            <header class="bhg-hero">
                ...
                <div class="bhg-hero-content">
                    ...
                    <?php if ( $address ) : ?>
                        <div class="bhg-address"><span aria-hidden="true">⌖</span>
                            <?php if ( $venue_url ) : ?><a href="<?php echo esc_url( $venue_url ); ?>"><?php echo esc_html( $address ); ?></a><?php else : echo esc_html( $address ); endif; ?>
                        </div>
                    <?php endif; ?>
            ...
            <div class="bhg-layout">
                <div class="bhg-main-column">
                    <section class="bhg-section bhg-quick-details">
                        <h2>Quick Details</h2>
                        <div class="bhg-detail-grid">
                            <?php foreach ( array( 'Age range' => $age, 'Price' => $price, 'Session length' => $session, 'Booking required' => $booking ) as $label => $value ) : ?>
                                <div class="bhg-detail-card"><span><?php echo esc_html( $label ); ?></span><strong><?php echo $value !== '' ? esc_html( $value ) : 'Not specified'; ?></strong></div>
                            <?php endforeach; ?>
                        </div>
                    </section>
                    ...
                    <section class="bhg-section bhg-map-section">
                        ...
                        <?php if ( count( $venues ) > 1 ) : ?>
                            <label class="bhg-venue-select-label" for="bhg-venue-select">Show classes at venue</label>
                            <select id="bhg-venue-select" class="bhg-venue-select" data-group-id="<?php echo esc_attr( $group_id ); ?>">
                                <option value="">Current venue</option>
                                <?php foreach ( $venues as $venue ) : ?><option value="<?php echo esc_attr( $venue->ID ); ?>" <?php selected( $venue->ID, $venue_id ); ?>><?php echo esc_html( $venue->post_title ); ?></option><?php endforeach; ?>
                            </select>
                        <?php endif; ?>
                        <?php if ( $map ) : ?><div id="bhg-map" class="bhg-map" data-lat="<?php echo esc_attr( $map['lat'] ); ?>" data-lng="<?php echo esc_attr( $map['lng'] ); ?>"></div><?php else : ?><div class="bhg-no-map">No map location has been added to this listing yet.</div><?php endif; ?>
                    </section>

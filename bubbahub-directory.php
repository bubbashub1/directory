...
function bubbahub_directory_badges( $post_id ) {
    ...
    } elseif ( is_string( $badges ) && trim( $badges ) !== '' ) $items = preg_split( '/[,|]+/', $badges );
...
function bubbahub_directory_shortcode( $atts ) {
    ...
    $args = array( 'post_type' => 'group', 'post_status' => 'publish', 'posts_per_page' => max( 1, min( 100, (int) $atts['posts_per_page'] ) ), 'paged' => max( 1, get_query_var( 'paged', 1 ) ), 's' => $search, 'orderby' => 'date', 'order' => 'DESC' );
    ...
    $age_values = array_unique( $age_values ); natcasesort( $age_values );
    ...
    <div class="bh-directory" data-default-columns="4">
        ...
        <div class="bh-toolbar"><div class="bh-results-count"><?php echo esc_html( number_format_i18n( $query->found_posts ) ); ?> groups</div><div class="bh-view-controls" role="group" aria-label="Directory view"><?php foreach ( array( 2,3,4,5,6 ) as $columns ) : ?><button type="button" class="bh-cols <?php echo $columns === 4 ? 'is-active' : ''; ?>" data-columns="<?php echo $columns; ?>" aria-label="<?php echo $columns; ?> columns"><?php echo $columns; ?></button><?php endforeach; ?><button type="button" class="bh-map-toggle" data-view="map">Map</button></div></div>
        <div class="bh-content"><div class="bh-grid" data-columns="4">
            <?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); $id = get_the_ID(); $image = bubbahub_directory_image_url( $id ); $map = bubbahub_directory_normalise_map( bubbahub_directory_get_field( $id, 'map' ) ); $region_name = bubbahub_directory_region( $id ); $age_range = bubbahub_directory_get_field( $id, 'age_range' ); $price_value = bubbahub_directory_get_field( $id, 'price' ); $badges = bubbahub_directory_badges( $id ); if ( is_array( $age_range ) ) $age_range = implode( ', ', $age_range ); if ( is_array( $price_value ) ) $price_value = implode( ', ', $price_value ); ?>
            <article class="bh-card" data-lat="<?php echo $map ? esc_attr( $map['lat'] ) : ''; ?>" data-lng="<?php echo $map ? esc_attr( $map['lng'] ) : ''; ?>" data-title="<?php echo esc_attr( get_the_title() ); ?>" data-url="<?php echo esc_url( get_permalink() ); ?>">
                <div class="bh-card-image"><?php if ( $image ) : ?><img src="<?php echo esc_url( $image ); ?>" alt="<?php echo esc_attr( get_the_title() ); ?>" loading="lazy"><?php else : ?><div class="bh-image-placeholder">BubbaHub</div><?php endif; ?><?php if ( $badges ) : ?><div class="bh-badges"><?php foreach ( $badges as $badge ) : ?><span><?php echo esc_html( $badge ); ?></span><?php endforeach; ?></div><?php endif; ?></div>
                <div class="bh-card-body"><h2><a href="<?php echo esc_url( get_permalink() ); ?>"><?php the_title(); ?></a></h2><?php if ( $region_name ) : ?><div class="bh-meta"><span class="bh-icon">⌖</span><?php echo esc_html( $region_name ); ?></div><?php endif; ?><div class="bh-card-meta"><?php if ( $age_range ) : ?><span><?php echo esc_html( $age_range ); ?></span><?php endif; ?><?php if ( $price_value ) : ?><span><?php echo esc_html( $price_value ); ?></span><?php endif; ?></div><a class="bh-view-more" href="<?php echo esc_url( get_permalink() ); ?>">View More <span aria-hidden="true">→</span></a></div>
            </article>
        ...
        </div><div class="bh-map" id="bh-directory-map" hidden aria-label="Map showing groups"></div></div>

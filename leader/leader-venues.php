...
function bubbahub_leader_venue_form( $post_id = 0 ) {
    ...
    <form class="bh-leader-form" method="post">
        <input type="hidden" name="bh_leader_action" value="save_venue">
...
function bubbahub_leader_handle_venue_save() {
    if ( empty( $_POST['bh_leader_action'] ) || 'save_venue' !== $_POST['bh_leader_action'] || ! is_user_logged_in() ) return;
    ...
    wp_safe_redirect( add_query_arg( 'venue_saved', '1', home_url( '/leader/#venues' ) ) );
    exit;
}

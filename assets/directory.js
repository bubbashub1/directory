...
function initDirectory($root){
 ...
 $root.on('click','.bh-map-toggle',function(){var show=$map.prop('hidden');if(show){$grid.hide();$map.prop('hidden',false).show();$(this).addClass('is-active').text('List');buildMap();setTimeout(function(){if(map)map.invalidateSize();},150);}else{$map.hide().prop('hidden',true);$grid.show();$(this).removeClass('is-active').text('Map');}});

...
function bubbahub_booking_reserve_ajax() {
    ...
    if ( ! $session_id || get_post_type( $session_id ) !== 'bh_session' ) {
        wp_send_json_error( array( 'message' => 'Invalid booking session.' ), 400 );
    }

    if ( ! $name || ! is_email( $email ) ) {
        wp_send_json_error( array( 'message' => 'Please enter your name and a valid email address.' ), 400 );
    }

    if ( ! (bool) bubbahub_booking_meta( $session_id, '_bh_reserve_enabled', false ) ) {
        wp_send_json_error( array( 'message' => 'Reservations are not enabled for this session.' ), 400 );
    }

...
function refresh($planner){
 ...
 $.post(cfg.ajaxUrl,{action:'bubbahub_myhub_planner',nonce:cfg.nonce,children:JSON.stringify(children),interest:interest,location:location,show_naps:showNaps},function(r){
   $results.html(r&&r.success?r.data.html:'<div class="bh-planner-empty">We could not update your planner. Please try again.</div>');
 },'json').fail(function(){$results.html('<div class="bh-planner-empty">We could not update your planner. Please try again.</div>');}).always(function(){$results.removeClass('bh-planner-refreshing');});
}
function init(){
 ...
 $hub.find('.bh-myhub-selected-child').each(function(){
   var id=$(this).data('child-id');
   var name=$(this).closest('.bh-myhub-child-card').find('h3').first().text()||'Child';
   var safeName=$('<div>').text(name).html();
   $pills.append('<label><input type="checkbox" data-planner-child value="'+String(id).replace(/"/g,'&quot;')+'" checked><span>'+safeName+'</span></label>');
 });
